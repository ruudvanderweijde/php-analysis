...
OFG prop() = createOFG(|project://eLibPartly|);

OFG createOFG(loc project) {
	Program p = getProgram(project); 
...
Program getProgram(loc project) = getProgram(createAstsFromEclipseProject(project, true));

Program getProgram(set[Declaration] asts) {
